...
export const createMouvement = async (req, res) => {
  ...
  try {
    ...
    const [materiel] = await connection.query('SELECT id FROM materiel WHERE id = ?', [materiel_id])
    ...
    const [depot] = await connection.query('SELECT id FROM depot WHERE id = ?', [depot_id])
    if (depot.length === 0) {
      await connection.rollback()
    ...
    if (depot_destination_id) {
      const [depotDest] = await connection.query('SELECT id FROM depot WHERE id = ?', [depot_destination_id])
      if (depotDest.length === 0) {
        await connection.rollback()
        return res.status(404).json({ error: 'Dépôt de destination non trouvé.' })
      }
      
      if (depot_id === depot_destination_id) {
        await connection.rollback()
        return res.status(400).json({ error: 'Le dépôt source et destination doivent être différents.' })
      }
    }
...
export const updateMouvement = async (req, res) => {
  ...
  try {
    ...
    const [existing] = await db.query('SELECT id FROM mouvement_stock WHERE id = ?', [id])
...
export const cancelMouvement = async (req, res) => {
  ...
  try {
    ...
    const [mouvement] = await connection.query('SELECT * FROM mouvement_stock WHERE id = ?', [id])
...
export const deleteMouvement = async (req, res) => {
  const { id } = req.params
  try {
    const [result] = await db.query('DELETE FROM mouvement_stock WHERE id = ?', [id])
    if (result.affectedRows === 0)
      return res.status(404).json({ message: 'Mouvement non trouvé.' })
